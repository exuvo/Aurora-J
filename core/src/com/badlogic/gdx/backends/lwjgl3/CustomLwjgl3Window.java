...
public class CustomLwjgl3Window extends Lwjgl3Window {
		...
		public void invoke(long windowHandle, final boolean iconified) {
			postRunnable(new Runnable() {
				...
				public void run() {
					...
					CustomLwjgl3Window.this.iconified = iconified;
					if (iconified) {
						listener.pause();
					} else {
						listener.resume();
	...
	/**
	 * Post a {@link Runnable} to this window's event queue. Use this if you access statics like {@link Gdx#graphics} in your runnable instead
	 * of {@link Application#postRunnable(Runnable)}.
	 */
	...
	/**
	 * Minimizes (iconifies) the window. Iconified windows do not call their {@link ApplicationListener} until the window is restored.
	 */
	 ...
	 * 
	 * @param image One or more images. The one closest to the system's desired size will be scaled. Good sizes include 16x16, 32x32 and
	 *          48x48. Pixmap format {@link Pixmap.Format.RGBA8888 RGBA8888} is preferred so the images will not have to be copied and
	 *          converted. The chosen image is copied, and the provided Pixmaps are not disposed.
	 */
	...
	/**
	 * Sets minimum and maximum size limits for the window. If the window is full screen or not resizable, these limits are ignored. Use -1 to
	...
	public boolean equals(Object obj) {
		...
		CustomLwjgl3Window other = (CustomLwjgl3Window) obj;

...
// @GuardedBy("this") could by used in the future.
	...
	private PooledConnection connectionInTransition;       // a PooledConnection which is currently within a PooledConnection.getConnection()
																									       // call, or null

	/**
	 * Thrown in {@link #getConnection()} or {@link #getValidConnection()} when no free connection becomes available within
	 ...
	 * <p>
	 * If <code>maxConnections</code> connections are already in use, the method waits until a connection becomes available or
	 * <code>timeout</code> seconds elapsed. When the application is finished using the connection, it must close it in order to return it to
	 * the pool.
	 *
	 ...
	 * <p>
	 * This method is slower than {@link #getConnection()} because the JDBC driver has to send an extra command to the database server to test
	 * the connection.
	 * <p>
...
// Purges the PooledConnection associated with the passed Connection from the connection pool.
	 ...
	 * <p>
	 * This is the number of internally kept recycled connections, for which <code>Connection.close()</code> has been called and which have
	 * not yet been reused.
	 *

...
//	val waveHeightWrapper = FloatBuffer.wrap(waveHeight)
	...
	class GravGlobalData(window: GravWindowData): Disposable {
		val shader: ShaderProgram = Assets.shaders["gravimetric"]!!
		val vertices: FloatArray
		val indices: ShortArray

		init {
			if (!shader.isCompiled || shader.getLog().isNotEmpty()) {
				println("shader gravimetric errors: ${shader.getLog()}")
				throw RuntimeException("Shader gravimetric compile error: ${shader.getLog()}")
			}
	...
	class GravWindowData(): Disposable {
		...
		val mesh: Mesh = Mesh(false, MAX_VERTICES, MAX_INDICES,
						...
						if (sensors.isNotEmpty()) {
							...
							sensors.forEachFast{ sensor ->
								if (partStates.isPartEnabled(sensor)) {
									val poweredState = partStates.getPartState(sensor)[PoweredPartState::class]
